...
| Natural-language parser/editor contracts | Passed | Full JVM suite and focused production Compose tests | Local JVM plus Medium Phone AVD, API 36 | 2026-08-26 | Deterministic bilingual/24-hour grammar, overlap recovery, Unicode category integrity, readiness/retry, Save access/freeze, cancellation, localized rendering, semantics, and 200% layout coverage |
| Natural-language connected product journey | Passed | Focused and full connected suites | Medium Phone AVD, Android 16/API 36 | 2026-08-26 | 5 product journeys plus 1 forced setup-failure regression: bilingual save/list/Room/live-alarm proof, exact-settings/inexact fallback, multi-locale parsing, exact recreation retention, and orphan-safe cleanup |
| Notification access boundary | Passed | Focused ViewModel tests, API 36 permission checker, and Compose warning semantics | JVM plus Medium Phone AVD, API 36 | 2026-08-26 | Parser/manual reminders share Save confirmation; denial leaves editor open and unpersisted; fresh denial and platform grant state passed |
| Exact-alarm inexact fallback | Passed | Natural-language save journeys and ViewModel scheduler contract | Medium Phone AVD, API 36 | 2026-08-26 | Exact access unavailable, settings opened, return callback retained warning state, and scheduled fallback alarm matched persisted reminder |
...
| Notification permission dialog and delivery | Pending | Tap deny/grant and deliver a reminder notification | API 33+ physical device | - | Platform dialog UX and notification posting remain manual |
| Exact-alarm grant and delivery timing | Pending | Grant capability and observe delivered exact reminder | API 33+ physical device | - | Emulator inexact fallback passed; grant and delivery remain manual |
